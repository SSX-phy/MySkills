...
- **[academic-sweep](academic-sweep/)** — citation-network sweep of an academic topic. Discovers an adaptive key-points template from one or more node papers, walks references and forward citations one hop with topical filtering, and produces `report.md` (synthesis) + `corpus.md` (audit table). Invoked by the `academic-explorer` subagent for Pattern A topic exploration.
